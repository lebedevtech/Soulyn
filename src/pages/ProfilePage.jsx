import { User, Settings, Star, MapPin, Edit3, Bug } from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { supabase } from '../lib/supabase';
import { useEffect, useState } from 'react';

export default function ProfilePage() {
  const { user, debugLogin } = useAuth(); // Используем функцию для смены
  ...
  return (
    <div className="relative w-full h-full bg-black overflow-y-auto no-scrollbar">
      ...
      <div className="relative z-10 pt-32 px-6 pb-32 flex flex-col items-center">
        ...
        {/* DEV PANEL (ТОЛЬКО ДЛЯ ТЕСТОВ) */}
        <div className="w-full mt-10 border-t border-white/10 pt-6">
            <h3 className="text-white/30 font-bold uppercase text-xs mb-4 flex items-center gap-2"><Bug size={14}/> Dev: Сменить аккаунт</h3>
            <div className="grid grid-cols-2 gap-2">
                {testUsers.map(u => (
                    <button 
                        key={u.id}
                        onClick={() => debugLogin(u.id)}
                        className={clsx("p-3 rounded-xl border text-xs font-bold text-left truncate transition-colors", user.id === u.id ? "bg-primary border-primary text-white" : "bg-white/5 border-white/10 text-white/50")}
                    >
                        {u.first_name || u.username || 'No Name'}
                    </button>
                ))}
            </div>
        </div>
