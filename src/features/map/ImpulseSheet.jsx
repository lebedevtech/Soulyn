...
const SHEET_TRANSITION = { duration: 0.4, ease: [0.25, 1, 0.5, 1] }; // Soft stop
    ...
    <AnimatePresence>
        <>
          ...
          <motion.div 
            ...
            animate={{ y: 0 }} 
            exit={{ y: "100%" }}
            transition={SHEET_TRANSITION} // Новая физика
                  ...
                  <div className="p-5 rounded-[24px] bg-white/5 border border-white/5 mb-6">
                     ...
                     {impulse.venues && (
                       <div className="mt-4 pt-4 border-t border-white/10 flex items-center gap-2 text-primary">
                         <MapPin size={16} /><span className="font-bold text-sm uppercase tracking-widest">{impulse.venues.name}</span>
                       </div>
                     )}
                  </div>

                  {isOwner ? (
                    <motion.button whileTap={{ scale: 0.98 }} onClick={() => setShowDeleteConfirm(true)} className="w-full py-4 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-500 font-bold text-lg flex items-center justify-center gap-2">
                      Удалить импульс <Trash2 size={20} />
                    </motion.button>
                  ) : (
                    requestStatus === 'pending' ? (
                      ...
                      <motion.button whileTap={{ scale: 0.98 }} onClick={handleJoin} disabled={loading} className="w-full py-4 rounded-2xl bg-primary text-white font-bold text-lg flex items-center justify-center gap-2 shadow-lg shadow-primary/20">
                        {loading ? '...' : <>Погнали! <CheckCircle size={20} /></>}
                      </motion.button>
                    )
                  )}
