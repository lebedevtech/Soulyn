...
export default function NotificationsPage() {
  ...
  const [items, setItems] = useState([]); // Смешанный список (мэтчи + сообщения)
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!user) return;
    const fetchData = async () => {
      // 1. Загружаем ЗАПРОСЫ НА МЭТЧ (pending)
      ...
      // 2. Загружаем НЕПРОЧИТАННЫЕ СООБЩЕНИЯ
      const { data: messages } = await supabase
        .from('messages')
        .select(`
            id, content, created_at, match_id,
            match:match_id (
      ...
      const formattedRequests = (requests || []).map(r => ({
        ...
        user: r.requester,
        text: `хочет встретиться на импульс "${r.impulse?.message}"`,
        raw: r
      }));

      // Форматируем сообщения
      const formattedMessages = (messages || []).map(m => {
        // Определяем, кто собеседник в этом матче
        const match = m.match;
  ...
  const handleAccept = async (matchId) => {
    ...
    setItems(prev => prev.filter(i => i.id !== matchId)); // Убираем из списка
  ...
  return (
    <div className="relative w-full h-full bg-black flex flex-col">
      ...
      <div className="absolute top-0 left-0 right-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-md border-b border-white/5 pt-[calc(env(safe-area-inset-top)+12px)] pb-3">
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-white/10 overflow-hidden relative">
                    ...
                    {item.type === 'message' && (
                        <div className="absolute bottom-0 right-0 w-3 h-3 bg-primary rounded-full border border-black" />
                    )}
                  </div>
                ...
                {item.type === 'request' ? (
                    ...
                    // Для сообщений - кнопка "Ответить"
                    <button 
                        onClick={() => navigate(`/chat/${item.matchId}`)}
                        className="w-full py-2.5 bg-white/5 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 active:scale-95 transition-transform hover:bg-white/10"
                    >
                        <MessageCircle size={16} /> Ответить
                    </button>
                )}
              </motion.div>
            ))}
