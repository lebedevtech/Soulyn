...
export default function ProfilePage() {
  ...
  if (isLoading) {
    return <div className="w-full h-full flex items-center justify-center bg-black"><div className="w-8 h-8 border-2 border-primary border-t-transparent rounded-full animate-spin" /></div>;
  }
    ...
    return (
      <div className="w-full h-full flex flex-col items-center justify-center bg-black text-white p-6 text-center">
        ...
        <div className="w-full max-w-xs space-y-2 mt-4">
            {testUsers.map(u => (
                <button key={u.id} onClick={() => debugLogin && debugLogin(u.id)} className="w-full p-3 bg-white/10 rounded-xl text-sm font-bold">{u.first_name}</button>
            ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="relative w-full h-full bg-black overflow-hidden flex flex-col">
      ...
      {/* z-50 чтобы быть поверх скролла */}
      <div className="absolute top-0 left-0 right-0 h-[52px] z-50 flex items-center justify-center">
        {/* Градиент под заголовком, чтобы текст читался на любом фоне */}
