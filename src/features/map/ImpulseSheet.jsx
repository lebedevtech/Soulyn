...
export default function ImpulseSheet({ impulse, onClose }) {
  ...
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false); // Состояние для подтверждения
  ...
  useEffect(() => {
    ...
    // Сбрасываем режим удаления при открытии нового импульса
  ...
  return (
    <AnimatePresence>
      ...
      >
        {/* ЕСЛИ РЕЖИМ УДАЛЕНИЯ - ПОКАЗЫВАЕМ ПОДТВЕРЖДЕНИЕ */}
        {showDeleteConfirm ? (
           <div className="p-8 pb-12 flex flex-col items-center text-center animate-in fade-in zoom-in duration-200">
             ...
             <p className="text-white/50 mb-8 font-medium">Это действие нельзя отменить. Импульс исчезнет с карты у всех пользователей.</p>
             
             <div className="w-full grid grid-cols-2 gap-3">
               <button 
                 onClick={() => setShowDeleteConfirm(false)}
                 className="py-4 rounded-2xl bg-white/5 font-bold text-white hover:bg-white/10 transition-colors"
               >
                 Отмена
               </button>
               <button 
                 ...
                 disabled={loading}
                 className="py-4 rounded-2xl bg-red-500 text-white font-bold flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(239,68,68,0.4)] active:scale-95 transition-transform"
               >
          ...
          <>
            ...
            <div className="px-6 pb-8">
              ...
              {isOwner ? (
                <button 
                  onClick={() => setShowDeleteConfirm(true)} // Включаем режим подтверждения
